...
int main()
{
    ...
    char sqlcmd[200];
    char ratec[5] = "", time[30] = "", status[10] = "";
    int r, t;
    int rate;
    ...
    sprintf(sqlcmd, "delete from disk");
    ...
    while (1)
    {
        ...
        while (1)
        {
            ...
            /*sprintf(sqlcmd, "delete from disk");
            mysql_real_query(mysql, sqlcmd, strlen(sqlcmd));*/
            sprintf(sqlcmd, "insert into disk values('%s', '%s', '%s')", ratec, time, status);
            mysql_real_query(mysql, sqlcmd, strlen(sqlcmd));

            dst = fopen("disk_recv.txt", "a+");
